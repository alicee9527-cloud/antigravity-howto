...
[![GitHub Stars](https://img.shields.io/github/stars/luongnv89/agy-howto?style=flat&color=gold)](https://github.com/luongnv89/agy-howto/stargazers)
[![GitHub Forks](https://img.shields.io/github/forks/luongnv89/agy-howto?style=flat)](https://github.com/luongnv89/agy-howto/network/members)
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![Version](https://img.shields.io/badge/version-2.1.206-brightgreen)](CHANGELOG.md)
[![Antigravity CLI](https://img.shields.io/badge/Antigravity_Code-2.1+-purple)](https://code.agy.com)
...
# Master Antigravity CLI in a Weekend

Go from typing `agy` to orchestrating agents, hooks, skills, and MCP servers — with visual tutorials, copy-paste templates, and a guided learning path.
...
- [How Antigravity How To Fixes This](#how-agy-how-to-fixes-this)
...
You installed Antigravity CLI. You ran a few prompts. Now what?
...
You're leaving 90% of Antigravity CLI's power on the table — and you don't know what you don't know.

---

## How Antigravity How To Fixes This

This isn't another feature reference. It's a **structured, visual, example-driven guide** that teaches you to use every Antigravity CLI feature with real-world templates you can copy into your project today.
...
Take the [self-assessment quiz](LEARNING-ROADMAP.md#-find-your-level) or run `/self-assessment` in Antigravity CLI. Get a personalized roadmap based on what you already know.
...
- **GitHub stars** from developers who use Antigravity CLI daily
- **Forks** from teams adapting this guide for their own workflows
- **Actively maintained** — synced with every Antigravity CLI release (latest: v2.1.206, July 2026)
- **Community-driven** — contributions from developers who share their real-world configurations

[![Star History Chart](https://api.star-history.com/svg?repos=luongnv89/agy-howto&type=Date)](https://star-history.com/#luongnv89/agy-howto&Date)
...
> **Installation note**: Starting in v2.1.113, Antigravity CLI ships as a native per-platform binary (macOS/Linux/Windows). `npm install -g @anthropic-ai/agy-code` still works — the native binary is downloaded as an optional dep on first use. As of v2.1.116, downloads come from `https://downloads.agy.ai/agy-code-releases` — corporate proxies must allowlist this host.

```bash
# 1. Clone the guide
git clone https://github.com/luongnv89/agy-howto.git
cd agy-howto

# 2. Copy your first slash command
mkdir -p /path/to/your-project/.agents/commands
cp 01-slash-commands/optimize.md /path/to/your-project/.agents/skills/

# 3. Try it — in Antigravity CLI, type:
# /optimize
...
**Is this maintained?**
Actively. The guide is synced with every Antigravity CLI release. Current version: v2.1.206 (July 2026), compatible with Antigravity CLI 2.1+.
...
**Can I use this with Antigravity Sonnet / Haiku / Opus?**
Yes. All templates work with Antigravity Sonnet 5, Antigravity Sonnet 4.6, Antigravity Opus 4.8, and Antigravity Haiku 4.5.
...
## Start Mastering Antigravity CLI Today

You already have Antigravity CLI installed. The only thing between you and 10x productivity is knowing how to use it. This guide gives you the structured path, the visual explanations, and the copy-paste templates to get there.
...
cp 06-hooks/*.sh ~/.gemini/antigravity-cli/hooks/
chmod +x ~/.gemini/antigravity-cli/hooks/*.sh
...
**Installation**:
```bash
cp 01-slash-commands/*.md /path/to/project/.agents/skills/
```
...
**Learn More**: [Discovering Antigravity CLI Slash Commands](https://medium.com/@luongnv89/discovering-agy-code-slash-commands-cdc17f0dfb29)
...
# Project skills
cp -r 03-skills/code-review-specialist /path/to/project/.agents/skills/
```
...
**Installation**:
```bash
cp 04-subagents/*.md /path/to/project/.agents/agents/
```
...
# Or add to project ~/.gemini/antigravity-cli/mcp_config.json manually (see 05-mcp/ for examples)
```

**Usage**: MCP tools are automatically available to Antigravity once configured
...
**What**: Event-driven shell commands that execute automatically in response to Antigravity CLI events
...
Configure hooks in `~/.gemini/antigravity-cli/settings.json`:
```json
{
  "hooks": {
    "PreToolUse": [{
      "matcher": "Write",
      "hooks": ["~/.gemini/antigravity-cli/hooks/format-code.sh"]
    }],
    "PostToolUse": [{
      "matcher": "Write",
      "hooks": ["~/.gemini/antigravity-cli/hooks/security-scan.sh"]
    }]
  }
}
```
...
- **Headless Mode** — Run Antigravity CLI in CI/CD: `agy -p "Run tests and generate report"`
- **Session Management** — `/resume`, `/rename`, `/fork`, `agy -c`, `agy -r`
- **Configuration** — Customize behavior in `~/.gemini/antigravity-cli/settings.json`
...
**What**: Complete command-line interface reference for Antigravity CLI

**Quick Examples**:
```bash
# Interactive mode
agy "explain this project"

# Print mode (non-interactive)
agy -p "review this code"

# Process file content
cat error.log | agy -p "explain this error"

# JSON output for scripts
agy -p --output-format json "list functions"

# Resume session
agy -r "feature-auth" "continue implementation"
```
...
4. Review Antigravity CLI version compatibility
...
This creates `agy-howto-guide.epub` with all content, including rendered Mermaid diagrams.
...
1. **Use GitHub Private Vulnerability Reporting**: https://github.com/luongnv89/agy-howto/security/advisories
...
- [Antigravity CLI Documentation](https://code.agy.com/docs/en/overview)
...
**Sources**:
- https://code.agy.com/docs/en/overview
- https://code.agy.com/docs/en/changelog
- https://code.agy.com/docs/en/permission-modes
- https://platform.agy.com/docs/en/about-agy/models/overview
- https://github.com/anthropics/agy-code/releases
